...
const articles = [
  ...
  {
    id: 'new-1772582249522',
    title: "Update Annual Pass Billing with Monthly Schedule",
    module: 'membership',
    role: 'all',
    ...
    relatedIds: [],
    media: [],
    video: "https://youtu.be/0O_Ug7rCXL0",
    content: "## Overview\nUse this procedure when an annual or platinum pass member doesn't owe a full year of dues and their billing needs to reflect only the months remaining. You'll check their ledger credit balance, look up the correct amounts in the Member Cheat Sheet, and enter those amounts into their billing tab.\n\nThe video will be easier to understand, please watch. \n---\n## What You'll Need Before You Start\n- The member's account pulled up in Club Caddie- Access to the **Member Cheat Sheet** — this is a separate Google Sheet (https://docs.google.com/spreadsheets/d/1AK1_V_ZZz6kczRXG7bvBtRxZecv9s02rSSAwHx7RFHo/edit?gid=142101542#gid=142101542) that has pre-calculated billing amounts for different numbers of months. \n---\n## Step 1: Pull Up the Member and Check Their Ledger\nSearch for the member by member number or name and double-click to open their profile.\n![](screenshots/member-search-results.png)\nClick the **Member Info** tab and confirm the membership expiration date. Count how many months remain between now and the expiration month — that's how many months you'll be billing for.\n*Example: The member's pass expires at the end of June. Today is in February, so the remaining months are March, April, May, and June — four months total.*\n![](screenshots/member-info-expiration.png)\nClick the **Ledger** tab and find the current credit balance. Write this number down — you'll use it in the next step to confirm you're looking at the right column in the cheat sheet.\n*Example: The ledger shows a credit balance of $985.46.*\n![](screenshots/ledger-credit-balance.png)\n---\n## Step 2: Look Up the Correct Amounts in the Member Cheat Sheet\nOpen the Member Cheat Sheet and find the column where the **Number of Months** row matches how many months your member still owes. Each column is a pre-calculated scenario — you're just finding the one that fits.\n*Example: Find the column where Number of Months = 4.*\nPull three values from that column:\n- **Ledger Amount** — Must match the credit balance you wrote down. If it doesn't, you're in the wrong column.\n\n    *Example: $985.46*\n\n- **Pre-Tax Amount** — The total dues before tax. You'll enter this into the billing tab.\n\n    *Example: $899.96*\n\n- **Monthly Amount** — The per-month dues amount. You'll enter this for each month the member still owes.\n\n    *Example: $224.99*\n\n![](screenshots/cheat-sheet-column-lookup.png)\n> **Note:** If the Ledger Amount in the cheat sheet doesn't match the credit balance from Club Caddie, you're in the wrong column — or the member's ledger may have already been partially updated. Stop and check with your manager before continuing.\n---\n## Step 3: Update the Billing Tab in Club Caddie\nGo back to the member's profile in Club Caddie. Click the **Membership** tab, then click **View/Edit Billing** to open the billing screen.\n![](screenshots/membership-view-edit-billing.png)\nIn the **top right corner** of the billing screen, find the total dues field. This is the pre-tax total. Enter the **Pre-Tax Amount** from the cheat sheet here.\n*Example: Enter $899.96 in the pre-tax total field.*\n![](screenshots/billing-pretax-field.png)\nNext, find the monthly amount fields — there is one row for each month of the year. Enter the **Monthly Amount** from the cheat sheet for each month the member still owes.\n*Example: Enter $224.99 for March, April, May, and June.*\n![](screenshots/billing-monthly-amounts.png)\nFor every other month — any month that has already been paid or that falls after the member's expiration date — enter **$0.00**.\n*Example: January and February have already been paid, so those are set to $0.00. All months after June are also $0.00.*\n![](screenshots/billing-zeroed-months.png)\nBefore saving, do a final check. Two totals on the billing screen should now match your cheat sheet values:\n- The **dues total** should equal the Pre-Tax Amount from the cheat sheet.  *Example: $899.96*- The **overall total** (which includes tax) should equal the Ledger Amount from the cheat sheet — and the credit balance you wrote down in Step 1.  *Example: $985.46*\nIf both numbers line up, click **Save** to apply the changes.\n![](screenshots/billing-totals-verified.png)\n> **Note:** If the numbers don't match, do not save. Go back to the cheat sheet and confirm you're in the correct column for the right number of months. Most mismatches come from being one column off.",
  },

  {
    id: 'new-1772582338414',
    title: "Update Individual Monthly Membership Billing Amount",
    module: 'membership',
    role: 'all',
    dateUpdated: '2026-03-03',
    version: '2.0',
    ...
    relatedIds: [],
    media: [],
    content: "# RTJ: Update Billing for Monthly Individual Annual Trail Pass Holders\n\n## Overview\n\nUse this procedure when a member holds a **Monthly Individual Annual Trail Pass** and their monthly billing amount needs to be corrected to match the pricing from the year they purchased.\n\nMembers who purchased their annual trail pass in 2025 locked in 2025 pricing. If their billing is currently showing a 2026 rate, it needs to be updated before their next charge runs. This is a quick fix — one field, one save.\n\n**The numbers in this article are examples.** Your member's amounts will be different depending on when they purchased and which pricing tier applies.\n\n---\n\n## What You'll Need Before You Start\n\n- The member's account pulled up in Club Caddie\n- The correct monthly rate for the member's purchase year — confirm this with your manager if you're unsure which rate applies\n\n---\n\n## Step 1: Pull Up the Member and Confirm Their Pass Details\n\nSearch for the member by name or member number and double-click to open their profile.\n\n![](screenshots/member-search-results.png)\n\nClick the **Member Info** tab and confirm two things:\n\n1. The membership type is **Monthly Individual Annual Trail Pass**\n2. The expiration date — this tells you which year's pricing the member is on\n\n*Example: Tommy's pass shows a Monthly Individual that expires at the end of July 2026.*\n\n![](screenshots/member-info-tab.png)\n\nClick the **Membership** tab and confirm the purchase date. Members who purchased in 2025 should be on 2025 pricing for the full term of their pass.\n\n*Example: Tommy purchased his pass in July 2025, so his monthly rate should reflect 2025 pricing — $205.00 per month, based on a total annual cost of $2,460.*\n\n![](screenshots/membership-tab-purchase-date.png)\n\n---\n\n## Step 2: Check the Current Billing Amount\n\nWhile still on the **Membership** tab, look at the monthly amount currently set for the member. If it shows anything other than their correct rate for their purchase year, it needs to be updated.\n\n*Example: Tommy's billing is currently set to $223.75 per month — which is the 2026 rate, not the 2025 rate he locked in. This needs to be corrected to $205.00.*\n\n![](screenshots/membership-current-monthly-amount.png)\n\n---\n\n## Step 3: Update the Monthly Billing Amount\n\nClick **View/Edit Billing** to open the billing screen.\n\n![](screenshots/view-edit-billing.png)\n\nFind the monthly amount field and replace the current amount with the correct rate for the member's purchase year.\n\n*Example: Change $223.75 to $205.00.*\n\n![](screenshots/billing-monthly-amount-updated.png)\n\nClick **Save**.\n\n![](screenshots/billing-save.png)\n\nThat's it. The member's billing is now set to the correct rate.\n\n---\n\n> **Note:** Every monthly individual who purchased their annual trail pass in 2025 needs this correction before the updated 2026 pricing takes effect. Work through your list of affected members and update each one using these same steps.",
  },

  {
    id: 'new-1772582388209',
    title: "Update Couples Membership Billing (Primary and Sub-Member)",
    module: 'membership',
    role: 'all',
    dateUpdated: '2026-03-03',
    version: '2.0',
    ...
    relatedIds: [],
    media: [],
    content: "## Overview\nThis article shows you how to update billing for couples memberships or annual trail pass holders. At Capitol Hill, couples have a primary account holder and a sub-member, each with their own member account number and card.\nThe critical rule: **Only the primary account holder gets billed. The sub-member billing must be set to zero** to avoid double-charging.\n---\n## Understanding Couples Memberships at Capitol Hill\n### How Couples Accounts Work\nFor couples memberships:- Each person has their own member account number- Each person has their own card with their own number- But only ONE person (the primary) should be billed- The other person (the sub-member) should have all billing set to $0.00\n**Why this matters:** If you don't zero out the sub-member's billing, the couple will be charged twice for the same membership.\n---\n## Step-By-Step:\n### Part 1: Set Up the Primary Account Holder\nFirst, you need to set up the primary account holder with the correct billing amounts. (Use the \"Update Annual Pass Billing with Monthly Schedule\" process for this.)\nOnce the primary account holder is set up correctly, move on to Part 2.\n### Part 2: Zero Out the Sub-Member Billing\nThis is the critical step that prevents double-charging.\n### Step 1: Find the sub-member account\nThe sub-member has their own member account number.\n**Example:** If the primary member is #30303219, the sub-member might be #30303220.\nSearch for and pull up the sub-member's profile.\n### Step 2: Double-click to open their profile\nOpen the sub-member's member profile.\n### Step 3: Check the Member Info tab\nClick the **Member Info** tab and verify:- They have an annual pass or couples membership- They do NOT have any sub-members on their account- This confirms they are the sub-member, not a primary account holder\n### Step 4: Go to the Membership tab\nClick the **Membership** tab.\n**What you'll see:**- There IS a billing amount on their profile (this is the problem)- Unlike the primary account holder, there is NO credit on their ledger\n**This tells you:** This person is set up to be billed, but they shouldn't be—the primary is already paying for both.\n### Step 5: Edit the billing amount\nIn the Membership tab, go into the billing section and edit the billing amounts.\nYou need to change all the monthly amounts to zero.\n### Step 6: Set all months to $0.00\nGo through each month and change the amount to **0.00** (zero):\n- January: $0.00- February: $0.00- March: $0.00- April: $0.00- May: $0.00- June: $0.00- July: $0.00- August: $0.00- September: $0.00- October: $0.00- November: $0.00- December: $0.00\n**This is very important.** We do not want to be double-charging accounts that have already paid for the annual couples membership or annual pass.\n### Step 7: Verify all amounts are zero\nBefore saving, double-check that you have **zeros across the board**. Every single month should show $0.00.\nMake sure everything looks good.\n### Step 8: Save the changes\nClick **Save**.\nYou're done! The sub-member will no longer be billed, and the couple will only be charged once (through the primary account holder).\n---\n## Important Notes\n### Why Sub-Members Must Be Zero\nThe primary account holder pays for the entire couples membership. If the sub-member also has billing amounts set up, the couple gets charged twice—once through each person's account.\nBy setting the sub-member to all zeros, you ensure the couple is only charged once.\n### Each Person Has Their Own Account\nAt Capitol Hill, sub-members aren't just names on the primary account—they have their own:- Member account number- Membership card- Profile in the system\nThis means you have to actively go in and zero out their billing. It won't happen automatically.\n### How to Tell Who's Primary vs. Sub-Member\n**Primary account holder:**- Has a credit balance on their ledger- Should have billing amounts set up- Pays for the membership\n**Sub-member:**- Has NO credit on their ledger- Should NOT have any billing amounts- Gets the benefit of the membership without paying directly\n### This Applies to All Couples Memberships\nWhether it's:- Annual trail pass couples- Platinum pass couples  - Any other couples membership type\nThe rule is the same: Primary pays, sub-member is zeroed out.\n---\n## Common Questions\n**What if I accidentally left billing amounts on a sub-member?**\nThe couple may have been double-charged. You'll need to:1. Zero out the sub-member billing immediately2. Check how many months they were double-charged3. Issue credits or refunds for the overcharges4. Notify the member\n**How do I know which member is primary and which is sub?**\nCheck the Ledger tab:- Primary = Has credit balance- Sub-member = No credit balance\nYou can also check with your manager or look at when the accounts were created.\n**What if both members have credit balances?**\nThis might indicate they were set up incorrectly. Check with your manager before proceeding.\n**Can the sub-member and primary swap roles?**\nTechnically yes, but you'd need to:1. Set up billing on the sub-member (who would become primary)2. Zero out the old primary (who would become sub-member)3. Transfer any credits appropriately\nThis is uncommon. Check with your manager if a couple requests this.\n**What if the sub-member wants their own separate individual membership?**\nThey would need to purchase a separate individual membership with their own account. They can't convert a couples sub-member account into a paying individual account without changing membership types.\n---\n## Troubleshooting\n**I zeroed out the sub-member but they're still being charged**\nCheck that:1. You saved the changes after entering zeros2. You zeroed out ALL months, not just some3. You're looking at the correct member account4. There aren't other billing items set up elsewhere\n**The system won't let me enter zero**\nTry entering **0.00** or **0** instead of leaving the field blank. Some systems require an actual zero value, not an empty field.\n**I'm not sure if I have the right sub-member**\nCheck the Member Info tab:- They should NOT have sub-members listed on their account- Their membership type should match the primary's membership type- They should NOT have a ledger credit balance\nIf you're still unsure, check with your manager.\n**What if there are more than 2 people on the membership?**\nSome memberships may include more than 2 people (like family memberships). The same rule applies:- ONE person is primary and gets billed- ALL other members should be zeroed out\nMake sure you zero out every sub-member, not just one.",
  },

  {
    id: 'new-1772653294042',
    title: "Clover Flex Beverage Cart Login and End-of-Shift Report",
    module: 'register',
    role: 'all',
    dateUpdated: '2026-03-04',
    version: '2.0',
    keywords: ["Clover Flex","beverage cart","bev cart","log in","login","passcode","employee login","Club Caddie app","role selection","terminal selection","Terminal Flex Utility","Beverage Cart role","Sales by Department","end-of-shift report","shift report","Food and Beverage login","shared login","day end closing","reports","employee filter"],
    relatedIds: [],
    media: [],
    content: "## Overview\nThis procedure covers how to log in to the Clover Flex handheld device for beverage cart operations, including unlocking the device, selecting the correct role and terminal, and logging in with your individual employee credentials. It also covers how a manager runs the end-of-shift Sales by Department report from the Club Caddie desktop application.\n> **Note:** Each staff member should use their own employee login on the Flex device. Do not use the shared Food & Beverage login that is set up for iPad use — that login is not intended for the Clover Flex.\n---\n## Log In to the Clover Flex\n**Step 1: Unlock the Device**\nUnlock the Flex device using passcode **1111**.\n![](screenshots/new-1772653294042-paste1.png)\n**Step 2: Open Club Caddie**\nSelect the **Club Caddie** app from the home screen.\n![](screenshots/new-1772653294042-paste2.png)\n**Step 3: Log In with Your Employee Credentials**\nLog in with your individual employee **username** and **password**.\n![](screenshots/new-1772653294042-paste3.png)\n**Step 4: Select Your Role and Terminal**\nSelect your role: **Beverage Cart**. Then select your terminal: **Terminal Flex Utility**. This terminal name is printed on a label on the physical device, so it's easy to match.\n![](screenshots/new-1772653294042-paste4.png)\n> **Note:** Using your own login — not a shared one — is what allows the end-of-shift report to break sales down by individual employee. It keeps records clean and reconciliation simple.\n---\n## Run the End-of-Shift Sales Report\nWhen your shift wraps up, go to Club Caddie Desktop and run a **Sales by Department** report to review what was sold from the beverage cart that day. This is done from the Club Caddie desktop application — not on the Flex device itself.\n**Step 5: Navigate to the Report**\nFrom the Club Caddie desktop application, navigate to **Reports → Sales by Department**.\n![](screenshots/new-1772655778792-paste5.png)\n**Step 6: Set the Filters**\nIn the first dropdown, select **Bev Cart**. In the third dropdown, select **Employee**.\n![](screenshots/new-1772653294042-paste5.png)\n**Step 7: Run the Report**\nRun the report. You'll see a detailed breakdown of that day's beverage cart sales by employee.\n\n",
  },
