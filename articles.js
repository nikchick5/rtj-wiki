...
const articles = [
...

  {
    ...
    relatedIds: [],
    media: [],
    content: "## Overview\nThis article shows you how to update billing for annual pass holders (including Platinum Pass) when you need to adjust their remaining monthly payments. You'll check their expiration date, find their ledger balance, calculate the monthly amounts using a spreadsheet, and set up a billing schedule for the remaining months.\nThis process is for members who have a credit balance on their ledger and need to pay monthly for the remainder of their membership period.\n---\n## Before You Start\nYou'll need:- Access to the member's profile- The calculation spreadsheet (provided)- The member's ledger balance- Their membership expiration date\n---\n## Step-By-Step:\n### Step 1: Open the member profile\n1. Pull up the member in Customers2. Double-click to open their profile\n**Example:** Member #40403469\n### Step 2: Check the Member Info tab to find the expiration date\n1. Click the **Member Info** tab2. Look at the expiration date3. Count how many months are left until expiration\n**Example:** If the member expires at the end of June, and it's currently March, they need to be billed for March, April, May, and June—that's **4 months**.\n**Why this matters:** You need to know how many months to spread the billing across.\n### Step 3: Go to the Ledger tab to find the credit balance\n1. Click the **Ledger** tab2. Look for their ledger balance (this will show as a credit)3. Write down this exact amount—you'll need it for the spreadsheet\n**Example:** Ledger balance shows $985.46\n### Step 4: Use the spreadsheet to calculate monthly amounts\nOpen the calculation spreadsheet. It has formulas built in to make this easy.\n**The spreadsheet has these columns:**- Number of Months- Ledger Amount- Pre-Tax Amount (calculated automatically)- Monthly Amount (calculated automatically)\n**Enter your information:**\n1. **Number of Months**: Enter how many months you counted in Step 2   - Example: 4\n2. **Ledger Amount**: Enter the amount from the Ledger tab   - Example: $985.46   - Make sure this matches exactly\n3. **Tax Rate**: The formula uses 1.095 (for 9.5% tax at Capitol Hill)   - You shouldn't need to change this unless your tax rate is different\n**The spreadsheet will automatically calculate:**- **Pre-Tax Amount**: Example: $899.96- **Monthly Amount**: Example: $224.99\n**Important:** The spreadsheet divides the ledger amount by 1.095 to remove the tax, then divides by the number of months. This gives you the pre-tax monthly amount to enter in Club Caddie.\n### Step 5: Go to the Membership tab\nClick the **Membership** tab to see their current membership details.\n### Step 6: Go to the Billing tab\n1. Click the **Billing** tab2. Click **View Edit Billing**\nThis opens the billing dialog where you'll enter the amounts.\n### Step 7: Enter the pre-tax amount in the top right field\nLook at the **Amount** field in the top right of the billing dialog.\n1. Go back to your spreadsheet and copy the **Pre-Tax Amount**   - Example: $899.96\n2. Paste it into the Amount field in the top right   - This is the total pre-tax value for all remaining months\n### Step 8: Fill in the Billing Schedule for remaining months\nNow scroll down to the **Billing Schedule** section. You'll see all 12 months listed.\n**For months they've already paid:**- Enter **0.00** (zero)- Example: If they already paid January and February, enter 0.00 for those months\n**For months they still need to pay:**- Go back to the spreadsheet and copy the **Monthly Amount**  - Example: $224.99- Paste this amount into each remaining month until expiration  - Example: March: $224.99, April: $224.99, May: $224.99, June: $224.99\n**For months after their membership expires:**- Enter **0.00** (zero)- Example: If they expire in June, enter 0.00 for July through December\n### Step 9: Verify the totals match\nAt the bottom of the Billing Schedule, check these three amounts:\n1. **Dues**: Should match the pre-tax amount from your spreadsheet   - Example: $899.96\n2. **Total Amount**: Should also match the pre-tax amount   - Example: $899.96\n3. **Ledger balance** (the top right Amount field): Should match what you saw in the Ledger tab   - Example: $985.46 (this includes the 9.5% tax)\n**If these don't match, double-check:**- You entered the monthly amount in the correct months- You zeroed out months already paid- You zeroed out months after expiration- Your spreadsheet calculation is correct\n### Step 10: Save the billing setup\nClick **Save** at the bottom of the dialog.\nYou're done! The member will now be billed the correct monthly amount for each remaining month.\n---\n## Important Notes\n### About the Tax Calculation\nThe spreadsheet divides by **1.095** to remove the 9.5% tax that Capitol Hill charges. This gives you the pre-tax amount to enter in Club Caddie.\nWhen the system charges the member each month, it will automatically add the 9.5% tax back on.\n**Example:**- You enter: $224.99 (pre-tax)- Member gets charged: $246.36 ($224.99 × 1.095)\n### If Your Tax Rate Is Different\nThe spreadsheet formula uses 1.095 for 9.5% tax. If your facility has a different tax rate, you'll need to adjust the formula:- 8% tax = 1.08- 10% tax = 1.10- etc.\n### Why We Zero Out Certain Months\n**Months already paid:** These members have a credit balance because they already paid for part of the year. You zero out the months they've already paid to avoid charging them twice.\n**Months after expiration:** If their membership expires in June, you don't want to charge them for July through December.\n### The Spreadsheet Has Examples\nThe calculation spreadsheet includes example calculations that you can reference. The formulas are built in, so you only need to:1. Enter the number of months2. Enter the ledger amount3. Adjust the tax rate if needed (usually you won't need to)\n---\n## Common Questions\n**What if the member's ledger balance doesn't match what I expected?**\nCheck the Ledger tab carefully. Make sure you're looking at the current credit balance, not a past transaction. If it still doesn't look right, verify with your manager before proceeding.\n**Can I change the monthly amounts later if needed?**\nYes. Go back to Billing > View Edit Billing, update the Billing Schedule, and click Save.\n**What if the member expires mid-month?**\nBill them through the last full month of their membership. For example, if they expire June 15th, bill them through June.\n**The totals don't match—what did I do wrong?**\nMost common issues:1. Forgot to zero out months already paid. Forgot to zero out months after expiration  3. Entered the wrong number of months in the spreadsheet4. Copied the wrong amount from the spreadsheet\nGo back and verify each month in the Billing Schedule.\n**What happens if their payment fails?**\nYou'll get a notification. You'll need to contact the member to update their payment method, then manually process any missed payments.\n---\n## Troubleshooting\n**I don't see \"View Edit Billing\" button**\nMake sure you're in the Billing tab. The button should be visible near the top of the billing information section.\n**The spreadsheet isn't calculating correctly**\nMake sure you:- Entered the number of months in the correct cell- Entered the ledger amount in the correct cell- Haven't accidentally deleted the formulas\n**The member was charged the wrong amount**\nCheck that you entered the **pre-tax** amount from the spreadsheet, not the ledger amount. The system adds tax automatically.",
  },
